...
#
# NAME
#   OracleJavaUpdate.sh -- Installs or updates Oracle Java
#
# SYNOPSIS
#   sudo OracleJavaUpdate.sh
#
####################################################################################################
#
# HISTORY
...
#   Version: 1.2
#
#   - v.1.0	Steve Miller, 2016.01.08:	Starting point used rtrouton oracle 8 script
#
####################################################################################################
...
if [ -f "$plugin" ]
then
    currentver=`/usr/bin/defaults read "/Library/Internet Plug-Ins/JavaAppletPlugin.plugin/Contents/Info.plist" CFBundleVersion`
    echo "Current version is: $currentver"
    currentvermain=${currentver:2:1}
    echo "Installed version main is: $currentvermain"
    currentvermin=${currentver:4:2}
    echo "Installed version minor is: $currentvermin"
    currentbuild=${currentver:7:2}
    echo "Installed version build is: $currentbuild"
    ...
    onlineversion2=${onlineversion1:50:12} 
    echo "Online version is: $onlineversion2"
    onlineversionmain=${onlineversion2:2:1}
    echo "Online version main is: $onlineversionmain"
    onlineversionmin=${onlineversion2:6:2}
    echo "Online version minor is: $onlineversionmin"
    onlineversionbuild=${onlineversion2:10:2}
    echo "Online version build is: $onlineversionbuild"
